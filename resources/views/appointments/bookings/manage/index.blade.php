...
@section('title', __('appointments.manage_bookings_title'))
...
@section('content')
    ...
    <div class="content-header row">
        <div class="content-header-left col-12 mb-2">
            <h3 class="content-header-title">@lang('appointments.manage_bookings_title')</h3>
            <div class="row breadcrumbs-top">
                <div class="breadcrumb-wrapper col-12">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') ?? '#' }}">@lang('appointments.breadcrumb_home')</a></li>
                        <li class="breadcrumb-item active">@lang('appointments.manage_bookings_title')</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <form method="GET" class="bk-filter mb-1">
        ...
        <div class="bk-row">
            <input type="text" name="q" class="form-control" value="{{ $filters['q'] }}" placeholder="@lang('appointments.filter_q_student')">
            ...
            <input type="date" name="date_from" class="form-control" value="{{ $filters['date_from'] }}" placeholder="@lang('appointments.filter_date_from')">
            <input type="date" name="date_to"   class="form-control" value="{{ $filters['date_to'] }}"   placeholder="@lang('appointments.filter_date_to')">
            <button type="submit" class="btn-gold"><i class="la la-search"></i> @lang('appointments.filter_apply')</button>
            <a href="{{ route('manage.appointments.index') }}" class="btn-reset"><i class="la la-times"></i> @lang('appointments.filter_reset')</a>
        </div>
    </form>
    ...
    <div class="bk-toolbar">
        <div class="left">
            ...
            @if($isAdmin)
                <span class="count-pill text-primary">@lang('appointments.label_all_school_bookings')</span>
            @endif
        </div>
    </div>
    <div class="bk-surface">
        @if($bookings->isEmpty())
            <div class="bk-empty">
                ...
                <h6>@lang('appointments.manage_empty_title')</h6>
                <p>@lang('appointments.manage_empty_hint')</p>
            </div>
        @else
            <div class="table-responsive">
                <table class="table bk-table">
                    <thead>
                        <tr>
                            <th>#</th>
                            <th>@lang('appointments.table_student')</th>
                            <th>@lang('appointments.table_booked_by')</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('manage.appointments.show', $booking->id) }}"
                                   class="btn btn-sm btn-outline-primary">
                                    <i class="la la-eye"></i> @lang('appointments.btn_show')
                                </a>
                            </td>
                        </tr>
                        @endforeach
