...
<div class="content-header row">
    <div class="content-header-left col-md-9 col-12 mb-2">
        <div class="row breadcrumbs-top">
            <div class="col-12">
                <h2 class="content-header-title float-{{ app()->getLocale() === 'ar' ? 'right' : 'left' }} mb-0">تعديل قسم</h2>
...
<div class="content-body">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('manage.sections.update', $section) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('gender') is-invalid @enderror" id="gender" name="gender" required>
                            <option value="male" {{ old('gender', $section->gender) == 'male' ? 'selected' : '' }}>بنين</option>
                            <option value="female" {{ old('gender', $section->gender) == 'female' ? 'selected' : '' }}>بنات</option>
                        </select>
                        @error('gender')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('level') is-invalid @enderror" id="level" name="level" required>
                            <option value="primary" {{ old('level', $section->level) == 'primary' ? 'selected' : '' }}>ابتدائي</option>
                            <option value="intermediate" {{ old('level', $section->level) == 'intermediate' ? 'selected' : '' }}>متوسط</option>
                            <option value="secondary" {{ old('level', $section->level) == 'secondary' ? 'selected' : '' }}>ثانوي</option>
                        </select>
                        @error('level')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="form-check form-switch">
                            ...
                            <input type="checkbox" class="form-check-input" id="is_active" name="is_active" value="1" {{ old('is_active', $section->is_active) ? 'checked' : '' }}>
