...
<section class="vc-ann" id="announcements">
    <div class="ls-header" style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.75rem;margin-bottom:1rem">
        <div>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.6rem;flex-wrap:wrap;gap:.5rem">
        <span class="badge badge-light">{{ number_format($announcements->total()) }} إعلان</span>
        <div style="display:flex;gap:.4rem">
            <button type="button" class="btn btn-sm btn-outline-secondary" data-toggle="dropdown"><x-svg-icon name="funnel" :size="14" /> تصفية</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="document.getElementById('annColsPanel').classList.toggle('d-none')"><x-svg-icon name="layout-three-columns" :size="14" /> تخصيص الأعمدة</button>
            <a href="{{ route('admin.announcements.index', array_merge(request()->query(), ['export' => 'csv'])) }}"
               class="btn btn-sm btn-outline-secondary"
               onclick="event.preventDefault(); exportAnnouncementsTable();"><x-svg-icon name="download" :size="14" /> تصدير إلى CSV</a>
        </div>
    </div>

    <div id="annColsPanel" class="card d-none" style="margin-bottom:.6rem">
    ...
    <div class="card">
        <div class="card-body" style="padding:0">
            @if($announcements->count() === 0)
                <div class="ls-empty" style="text-align:center;padding:3rem 1rem">
                    <div class="ls-empty-ico" style="margin-bottom:.5rem"><x-svg-icon name="megaphone-fill" :size="40" class="ic-muted" /></div>
                    <h4>لا توجد إعلانات</h4>
                    <p class="text-muted">لم يتم إنشاء أي إعلان بعد. ابدأ بإضافة إعلان جديد.</p>
                    @if($user->canDo('announcements.create'))
                        <a href="{{ route('admin.announcements.create') }}" class="btn btn-primary" style="margin-top:.5rem"><x-svg-icon name="plus-lg" :size="16" /> إضافة إعلان</a>
                    @endif
                </div>
            @else
                        ...
                        @foreach($announcements as $a)
                            ...
                            <tr>
                                ...
                                <td data-col="title">
                                    ...
                                    @if($a->type !== 'normal')
                                        <span class="badge badge-{{ $a->type === 'important' ? 'warning' : 'info' }}" style="margin-inline-start:.3rem">{{ $typeLbl }}</span>
                                    @endif
                                </td>
                                <td data-col="target">{{ $targetLabels[$a->target_type] ?? $a->target_type }}</td>
                                ...
                                <td data-col="status"><span class="badge badge-{{ $sm[1] }}"><x-svg-icon :name="$sm[2]" :size="12" /> {{ $sm[0] }}</span></td>
                                <td data-col="creator">{{ optional($a->creator)->name ?? '—' }}</td>
                                <td data-col="created">{{ $a->created_at?->format('Y-m-d') }}</td>
                                <td>
                                    <div class="dropdown">
                                        <button type="button" class="btn btn-sm btn-icon" data-toggle="dropdown"><x-svg-icon name="three-dots-vertical" :size="16" /></button>
                                        <div class="dropdown-menu dropdown-menu-end">
                                            @if($user->canDo('announcements.view'))
                                                <a class="dropdown-item" href="{{ route('admin.announcements.show', $a->id) }}"><x-svg-icon name="eye-fill" :size="14" /> عرض</a>
                                            @endif
                                            @if($user->canDo('announcements.edit'))
                                                <a class="dropdown-item" href="{{ route('admin.announcements.edit', $a->id) }}"><x-svg-icon name="pencil-square" :size="14" /> تعديل</a>
                                            @endif
                                            ...
                                            @if($user->canDo('announcements.read_log'))
                                                <a class="dropdown-item" href="{{ route('admin.announcements.read-log', $a->id) }}"><x-svg-icon name="list-check" :size="14" /> سجل القراءة</a>
                                            @endif
                                            @if($user->canDo('announcements.delete'))
                                                ...
                                                <form method="POST" action="{{ route('admin.announcements.destroy', $a->id) }}" onsubmit="return confirm('هل أنت متأكد من حذف هذا الإعلان؟')">@csrf @method('DELETE')
                                                    <button class="dropdown-item text-danger" type="submit"><x-svg-icon name="trash3-fill" :size="14" /> حذف</button>
                                                </form>
                                            @endif
                                        </div>
                                    </div>
                ...
                @if($announcements->hasPages())
                    <div style="padding:.85rem 1rem;border-top:1px solid #f1f5f9">{{ $announcements->links() }}</div>
                @endif
            @endif
        </div>
    </div>
