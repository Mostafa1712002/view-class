...
<div class="content-header-right col-md-5 text-md-right"><a href="{{ route('admin.qr.groups.create') }}" class="btn btn-primary btn-sm"><i class="la la-plus"></i> إنشاء مجموعة</a></div></div>
<div class="content-body">
    ...
    <div class="card mb-3"><div class="card-body"><form method="GET" class="form-row align-items-end">
        ...
        <div class="col-md-3 mb-2"><button class="btn btn-primary"><i class="la la-search"></i> بحث</button> <a href="{{ route('admin.qr.groups.index') }}" class="btn btn-outline-secondary">إعادة تعيين</a></div>
    </form></div></div>

    <div class="card"><div class="card-body table-responsive">
        @if($groups->isEmpty())<div class="text-center text-muted py-5"><i class="la la-layer-group la-3x d-block mb-2"></i> لا توجد مجموعات. أنشئ مجموعة جديدة.</div>
        @else
        ...
        <tbody>@foreach($groups as $g)<tr>
            ...
            <td>
                <a href="{{ route('admin.qr.groups.edit', $g->id) }}" class="btn btn-sm btn-outline-primary"><i class="la la-edit"></i></a>
                <form method="POST" action="{{ route('admin.qr.groups.destroy', $g->id) }}" class="d-inline" onsubmit="return confirm('حذف المجموعة؟');">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="la la-trash"></i></button></form>
            </td>
