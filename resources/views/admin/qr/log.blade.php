...
<div class="content-body">
    ...
    <div class="card mb-3"><div class="card-body"><form method="GET" class="form-row align-items-end">
        ...
        <div class="col-md-3 mb-2"><button class="btn btn-primary"><i class="la la-search"></i> بحث</button> <a href="{{ route('admin.qr.log') }}" class="btn btn-outline-secondary">إعادة تعيين</a></div>
    </form></div></div>

    <div class="card mb-3"><div class="card-body table-responsive">
        @if($scans->isEmpty())<div class="text-center text-muted py-5"><i class="la la-list la-3x d-block mb-2"></i> لا توجد مسحات مطابقة.</div>
        @else
    ...
    <div class="card border-warning"><div class="card-body">
        <h5 class="mb-3"><i class="la la-lock"></i> إغلاق اليوم</h5>
        <form method="POST" action="{{ route('admin.qr.close-day') }}" class="form-row align-items-end" onsubmit="return confirm('سيتم منع تسجيل مسحات جديدة وتسجيل غير الممسوحين كغياب. متابعة؟');">
            ...
            <div class="col-md-3 mb-2"><button class="btn btn-warning"><i class="la la-lock"></i> تنفيذ إغلاق اليوم</button></div>
        </form>
    </div></div>
</div>
@endsection
