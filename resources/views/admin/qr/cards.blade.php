...
<div class="content-header-right col-md-5 text-md-right">
    <a href="{{ route('admin.qr.scanner') }}" class="btn btn-outline-primary btn-sm"><i class="la la-qrcode"></i> ماسح QR</a>
    <a href="{{ route('admin.qr.log') }}" class="btn btn-outline-secondary btn-sm"><i class="la la-list"></i> سجل المسحات</a>
    <a href="{{ route('admin.qr.groups.index') }}" class="btn btn-outline-secondary btn-sm"><i class="la la-layer-group"></i> المجموعات</a>
</div></div>
<div class="content-body">
    ...
    <div class="card mb-3"><div class="card-body"><form method="GET" class="form-row align-items-end">
        ...
        <div class="col-md-3 mb-2"><button class="btn btn-primary"><i class="la la-search"></i> بحث</button></div>
    </form></div></div>

    <div class="card"><div class="card-body table-responsive">
        @if($students->isEmpty())<div class="text-center text-muted py-5"><i class="la la-id-card la-3x d-block mb-2"></i> ابحث لعرض الطلاب وبطاقاتهم.</div>
        @else
        ...
        <tr>
            ...
            <td>
                <form method="POST" action="{{ route('admin.qr.cards.generate') }}" class="d-inline">@csrf<input type="hidden" name="student_id" value="{{ $s->id }}">
                    <button class="btn btn-sm btn-outline-primary" title="{{ $card?'إعادة توليد':'إنشاء' }}"><i class="la la-qrcode"></i></button>
                </form>
                @if($card)
                <form method="POST" action="{{ route('admin.qr.cards.toggle', $card->id) }}" class="d-inline">@csrf
                    <button class="btn btn-sm btn-outline-{{ $card->is_active?'warning':'success' }}"><i class="la la-power-off"></i></button>
                </form>
                <a href="{{ route('admin.qr.cards.print', ['ids'=>[$card->id]]) }}" target="_blank" class="btn btn-sm btn-outline-secondary"><i class="la la-print"></i></a>
                @endif
            </td>
        </tr>
