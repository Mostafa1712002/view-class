...
<div class="content-body">
    ...
    @if(empty($setting->api_key))
        <div class="alert alert-warning d-flex align-items-center gap-2" role="alert">
            ...
            <span>@lang('sms_services.configure_credentials_empty')</span>
        </div>
    @endif
    ...
    <div class="row">
        <div class="col-lg-8">
            {{-- Connection settings --}}
            <div class="ds-card card mb-3">
                <div class="ds-card-header card-header">
                    <h5 class="ds-card-title mb-0">
                        <x-svg-icon name="plug" :size="16" class="me-1" /> @lang('sms_services.edit_connection')
                    </h5>
                </div>
                    ...
                    <form action="{{ route('admin.sms-services.connection.update', $school) }}" method="POST">
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <x-svg-icon name="check-circle" :size="14" class="me-1" /> @lang('sms_services.save_settings')
                            </button>
            ...
            <div class="ds-card card">
                <div class="ds-card-header card-header">
                    <h5 class="ds-card-title mb-0">
                        <x-svg-icon name="check-lg" :size="16" class="me-1" /> @lang('sms_services.test_connection')
                    </h5>
                </div>
                <div class="ds-card-body card-body">
                    <form action="{{ route('admin.sms-services.connection.test', $school) }}" method="POST" class="d-inline">
                        ...
                        <button type="submit" class="btn btn-outline-secondary">
                            <x-svg-icon name="plug" :size="14" class="me-1" /> @lang('sms_services.test_connection')
                        </button>
                    </form>
                    <p class="text-muted small mt-2 mb-0">@lang('sms_services.test_connection_hint')</p>
                </div>
            </div>
        </div>
    </div>
