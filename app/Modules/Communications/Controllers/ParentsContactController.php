...
class ParentsContactController extends Controller
{
    ...
    public function __construct(private readonly ParentsContactRepository $parents)
    {
    }
    ...
    {
        $parent = $this->parents->findScoped($id, $this->scopedSchoolId());
    ...
    public function export(Request $request): StreamedResponse
    {
        ...
        $rows = [[
            ...
            'عدد الأبناء', 'عدد رسائل البريد', 'عدد رسائل واتساب', 'عدد الإشعارات',
        ]];
        foreach ($parents as $p) {
            $rows[] = [
                $p->name, $p->nationality, $p->phone,
                $p->children_count ?? 0, $p->mail_count ?? 0,
