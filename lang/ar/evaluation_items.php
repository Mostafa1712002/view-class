<?php

return [
    // ── Items (العناصر) ─────────────────────────────────────────────
    'items' => [
        'page_title'   => 'معايير النموذج',
        'subtitle'     => 'إدارة معايير التقييم وأوزانها النسبية',
        'add'          => 'إضافة معيار',
        'edit'         => 'تعديل المعيار',
        'none'         => 'لا توجد معايير بعد',
        'none_hint'    => 'ابدأ بإضافة أول معيار للنموذج.',
        'back_to_form' => 'عودة لبيانات النموذج',
        'indicators'   => 'المؤشرات',
        'weight_total' => 'إجمالي الأوزان',
        'weight_remaining' => 'المتبقي',
        'of_100'       => 'من 100%',
        'columns' => [
            'order'            => '#',
            'name'             => 'معيار',
            'responsible_role' => 'الجهة المسؤولة',
            'item_type'        => 'النوع',
            'weight'           => 'الوزن',
            'max_score'        => 'أعلى درجة',
            'required'         => 'إجباري',
            'evidence'         => 'الشواهد',
            'indicators'       => 'المؤشرات',
            'status'           => 'الحالة',
            'actions'          => 'التحكم',
        ],
        'fields' => [
            'name'             => 'معيار',
            'description'      => 'الوصف',
            'weight'           => 'الوزن النسبي (%)',
            'max_score'        => 'أعلى درجة',
            'sort_order'       => 'الترتيب',
            'is_required'      => 'معيار إجباري',
            'needs_evidence'   => 'يتطلب شواهد',
            'evidence_required'=> 'الشواهد إلزامية',
            'allow_note'       => 'السماح بملاحظة',
            'visible_to_evaluator_only'       => 'مرئي للمقيّم فقط',
            'visible_to_subject_after_result' => 'مرئي لمن يُقيَّم بعد النتيجة',
            'status'           => 'الحالة',
            // Phase A (v2)
            'responsible_role'               => 'الجهة المسؤولة',
            'responsible_role_placeholder'   => 'مثال: school-admin، supervisor',
            'responsible_user'               => 'الجهة المسؤولة',
            'responsible_user_placeholder'   => 'اختر الحساب المسؤول',
            'item_type'                      => 'نوع المعيار',
            'calc_method'                    => 'طريقة الاحتساب',
            'evidence_needs_approval'        => 'الشواهد تحتاج موافقة',
            'editable_after_review'          => 'قابل للتعديل بعد الإرسال للمراجعة',
            'editable_after_approval'        => 'قابل للتعديل بعد الاعتماد',
            'min_percentage'                 => 'الحد الأدنى (%)',
            'min_percentage_placeholder'     => 'اتركه فارغاً إن لم يكن هناك حد أدنى',
            'internal_notes'                 => 'ملاحظات داخلية',
            'internal_notes_placeholder'     => 'ملاحظات لا تظهر للمقيّم',
        ],
        'advanced_config' => 'الإعدادات المتقدمة للمعيار',
        'indicators_section' => 'المؤشرات والنسب',
        'add_indicator'      => 'إضافة مؤشر',
        'indicator_text'     => 'نص المؤشر',
        'indicator_weight'   => 'النسبة (%)',
        'indicators_hint'    => 'يوزَّع الوزن النسبي للمعيار على مؤشراته؛ مجموع النسب يجب ألا يتجاوز وزن المعيار.',
        'indicators_sum'     => 'مجموع النسب',
    ],

    // ── Indicators (المؤشرات) ──────────────────────────────────────
    'indicators' => [
        'page_title' => 'مؤشرات المعيار',
        'subtitle'   => 'إدارة مؤشرات هذا المعيار',
        'add'        => 'إضافة مؤشر',
        'edit'       => 'تعديل المؤشر',
        'none'       => 'لا توجد مؤشرات بعد',
        'none_hint'  => 'ابدأ بإضافة أول مؤشر لهذا المعيار.',
        'back_to_items' => 'عودة للمعايير',
        'for_item'   => 'المعيار',
        'columns' => [
            'order'    => '#',
            'text'     => 'نص المؤشر',
            'level'    => 'المستوى',
            'required' => 'إجباري',
            'note'     => 'ملاحظة',
            'evidence' => 'الشواهد',
            'status'   => 'الحالة',
            'actions'  => 'التحكم',
        ],
        'fields' => [
            'text'             => 'نص المؤشر',
            'description'      => 'الوصف',
            'level_id'         => 'المستوى (روبرك)',
            'level_placeholder'=> 'اختر المستوى',
            'sort_order'       => 'الترتيب',
            'is_required'      => 'مؤشر إجباري',
            'needs_note'       => 'يتطلب ملاحظة',
            'needs_evidence'   => 'يتطلب شواهد',
            'evidence_required'=> 'الشواهد إلزامية',
            'weight'           => 'النسبة (%)',
            'status'           => 'الحالة',
        ],
        'level_only_rubric' => 'ربط المستوى متاح لنماذج الروبرك فقط.',
    ],

    // ── Item types (Phase A v2) ──────────────────────────────────────
    'item_types' => [
        'manual'        => 'يدوي',
        'auto'          => 'تلقائي',
        'evidence_only' => 'شواهد فقط',
        'mixed'         => 'مختلط',
    ],

    // ── Calc methods (Phase A v2) ────────────────────────────────────
    'calc_methods' => [
        'manual'         => 'يدوي',
        'auto_platform'  => 'تلقائي من المنصة',
        'after_evidence' => 'بعد اعتماد الشواهد',
        'external'       => 'خارجي',
    ],

    // ── Shared ──────────────────────────────────────────────────────
    'status' => [
        'active'   => 'مفعّل',
        'disabled' => 'معطّل',
    ],
    'actions' => [
        'save'    => 'حفظ',
        'cancel'  => 'إلغاء',
        'edit'    => 'تعديل',
        'delete'  => 'حذف',
        'enable'  => 'تفعيل',
        'disable' => 'تعطيل',
    ],
    'messages' => [
        'form_not_found'      => 'النموذج غير موجود.',
        'item_not_found'      => 'المعيار غير موجود.',
        'indicator_not_found' => 'المؤشر غير موجود.',
        'item_created'        => 'تم إضافة المعيار بنجاح.',
        'item_updated'        => 'تم حفظ تعديلات المعيار.',
        'item_deleted'        => 'تم حذف المعيار.',
        'item_disabled'       => 'تم تعطيل المعيار.',
        'item_enabled'        => 'تم تفعيل المعيار.',
        'indicator_created'   => 'تم إضافة المؤشر بنجاح.',
        'indicator_updated'   => 'تم حفظ تعديلات المؤشر.',
        'indicator_deleted'   => 'تم حذف المؤشر.',
        'indicator_disabled'  => 'تم تعطيل المؤشر.',
        'indicator_enabled'   => 'تم تفعيل المؤشر.',
        'reordered'           => 'تم حفظ الترتيب.',
        'delete_blocked_item' => 'لا يمكن حذف معيار مُستخدم في تقييم فعلي — يمكن تعطيله فقط.',
        'delete_blocked_indicator' => 'لا يمكن حذف مؤشر مُستخدم في تقييم فعلي — يمكن تعطيله فقط.',
        'form_locked'         => 'لا يمكن تعديل بنية نموذج منشور أو مغلق.',
        'delete_confirm'      => 'هل أنت متأكد من الحذف؟',
    ],
    'errors' => [
        'weight_over_100'        => 'مجموع أوزان المعايير سيتجاوز 100% (المتبقي المتاح: :remaining%).',
        'required_zero_weight'   => 'لا يمكن أن يكون المعيار الإجباري بوزن صفر.',
        'level_required_rubric'  => 'يجب اختيار المستوى لنماذج الروبرك.',
        'level_invalid'          => 'المستوى المختار لا يخص هذا النموذج.',
        'indicators_over_weight' => 'مجموع نسب المؤشرات (:sum%) يتجاوز الوزن النسبي للمعيار (:weight%).',
    ],
];
