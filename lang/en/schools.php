<?php

return [
    // Headings
    'title' => 'Schools',
    'add_school' => 'Add School',
    'edit_school' => 'Edit School',
    'school_details' => 'School Details',
    'branches' => 'Branches',
    'extra_services' => 'Extra Services',
    'student_gender' => 'Student Gender',
    'gender_boys' => 'Boys',
    'gender_girls' => 'Girls',
    'gender_mixed' => 'Mixed',
    'timezone' => 'Local Timezone',
    'track_predefined_hint' => 'Tracks are system-defined values',
    'no_branches_yet' => 'No branches yet.',
    'add_branch_now' => 'Add a branch now',

    // Form fields
    'name' => 'School Name',
    'name_ar' => 'Name (Arabic)',
    'name_en' => 'Name (English)',
    'branch' => 'Branch',
    'sort_order' => 'Sort Order',
    'educational_track' => 'Educational Track',
    'track_national' => 'National',
    'track_international' => 'International',
    'track_general' => 'General',
    'track_k12' => 'K12',
    'stage' => 'Stage',
    'city' => 'City',
    'default_language' => 'Default Language',
    'lang_ar' => 'Arabic',
    'lang_en' => 'English',
    'educational_company' => 'Educational Company',
    'fax' => 'Fax',
    'website' => 'Website',
    'logo' => 'School Logo',
    'logo_hint' => 'Max 2 MB',
    'social_accounts' => 'Social Accounts',
    'is_active' => 'Active',

    // Index columns
    'sections_count' => 'Sections',
    'classes_count' => 'Classes',
    'students_count' => 'Students',
    'licensed_students_count' => 'Licensed Students',

    // Control menu
    'control_menu' => 'Control Menu',
    'general_settings' => 'General Settings',
    'academic_years' => 'Academic Years',
    'grade_levels' => 'Grade Levels',
    'permissions' => 'Permissions',

    // Settings
    'settings_user_edit_perms' => 'User Edit Permissions',
    'settings_user_edit_perms_hint' => 'Choose which fields each role can edit on their own profile',
    'settings_weekly_plan' => 'Weekly Plan Settings',
    'settings_class_reports' => 'Class Settings',
    'settings_discussion_rooms' => 'Discussion Rooms',
    'settings_grade_reports' => 'Grade Reports',

    'user_role_student' => 'Student',
    'user_role_parent' => 'Parent',
    'user_role_teacher' => 'Teacher',
    'user_role_other' => 'Other Users',
    'user_field_username' => 'Username',
    'user_field_password' => 'Password',
    'user_field_name' => 'Name',
    'user_field_avatar' => 'Avatar',
    'user_field_phone' => 'Phone',
    'user_field_email' => 'Email',

    'classes_allow_overlap' => 'Allow overlapping class periods',
    'classes_show_header_footer' => 'Show header and footer in reports',

    'week_start_day' => 'Week Start Day',
    'working_days' => 'Working Days',
    'calendar_type' => 'Calendar Type',
    'calendar_gregorian' => 'Gregorian',
    'calendar_hijri' => 'Hijri',
    'target_homework_per_teacher' => 'Target homework per teacher',
    'weekly_plan_print_header' => 'Print header',
    'weekly_plan_print_footer' => 'Print footer',
    'lock_student_reassign' => 'Lock student reassignment / schedule deletion',

    'discussion_teacher_create' => 'Allow teachers to create discussion rooms',
    'discussion_student_post' => 'Allow students to open topics',
    'discussion_max_replies' => 'Max replies per student',

    'gpa_system' => 'GPA system',
    'gpa_100' => '100-point',
    'gpa_5' => '5-point',
    'gpa_4' => '4-point',

    'virtual_external_url' => 'External URL',
    'virtual_student_direct' => 'Students can enter the class directly through the platform',
    'whatsapp_enabled' => 'Enable WhatsApp service',

    'settings_school_info' => 'School Information',
    'school_active' => 'School Status',
    'settings_attendance' => 'Attendance Settings',
    'attendance_auto_biometric' => 'Calculate attendance / absence / lateness automatically from biometric',
    'attendance_auto_notify' => 'Send attendance / lateness notifications to parents automatically',
    'settings_messages' => 'Message Settings',
    'messages_supervisors_view' => 'Allow supervisors to view messages between teachers and students',
    'messages_students_to_classmates' => 'Allow students to message their classmates in the same class',
    'settings_virtual_classes' => 'Virtual Classes',
    'virtual_platform' => 'Platform',
    'virtual_external_link' => 'External link',
    'virtual_zoom_note' => 'Enabling Zoom requires contacting support',
    'settings_whatsapp' => 'WhatsApp Settings',
    'whatsapp_host' => 'Host',
    'whatsapp_token' => 'Token',

    // Academic years sub-screen
    'add_academic_year' => 'Add Academic Year',
    'promote_to_new_year' => 'Promote to New Year',
    'migrate_title' => 'Academic Year Migration',
    'migrate_type' => 'Migration type',
    'migrate_type_classes' => 'Migrate classes',
    'migrate_type_students' => 'Migrate students',
    'migrate_source' => 'Source',
    'migrate_destination' => 'Destination',
    'migrate_source_year' => 'Source academic year',
    'migrate_destination_year' => 'Destination academic year',
    'migrate_grade' => 'Grade level',
    'migrate_class' => 'Class',
    'migrate_run' => 'Run migration',
    'migrate_pick_class' => 'Select class',
    'migrate_no_classes' => 'No matching classes',
    'migrate_confirm' => 'Data will be copied to the selected destination. The source data is kept intact. Are you sure?',
    'migrate_classes_result' => 'Classes migration: :created created, :skipped skipped (already present).',
    'migrate_students_result' => 'Students migration: :migrated migrated, :skipped skipped (already present).',
    'migrate_timeslots_note' => 'Time slots are school-wide and used automatically across all years, so they need no migration.',
    'migrate_lessons_note' => 'Lessons migration is handled by the lessons module.',
    'study_weeks' => 'Study Weeks',
    'academic_year' => 'Academic Year',
    'year_start' => 'Start',
    'year_end' => 'End',
    'current' => 'Current',
    'current_year' => 'Current Academic Year',
    'set_as_current' => 'Set as current',
    'set_current' => 'Set current',
    'terms' => 'Terms',
    'term_name' => 'Term name',
    'add_term' => 'Add term',
    'weeks_count' => 'Weeks',
    'week_name' => 'Week name',
    'add_week' => 'Add week',
    'no_terms_yet' => 'No terms yet',
    'no_years_yet' => 'No academic years yet',
    'year_created' => 'Academic year created',
    'term_created' => 'Term created',
    'term_set_current' => 'Term set as current',
    'week_created' => 'Week added',
    'confirm_promote' => 'Promotion to a new year will start. Continue?',
    'promote_stub_note' => 'Promotion request received. Full migration (students/sections/schedules) will land in the next release.',

    // Grade levels sub-screen
    'add_section' => 'Add Section',
    'assign_grade_levels' => 'Assign Grade Levels',
    'add_grade_level' => 'Add Grade Level',
    'grade_level_name' => 'Grade Level Name',
    'grade_level_name_hint' => 'e.g. Grade 1 Primary',
    'grade_level_number' => 'Grade #',
    'grade_type' => 'Grade Type',
    'grade_number_taken' => 'This grade number is already used in this school.',
    'map_to_standard' => 'Map to standard…',
    'class_number' => 'Class #',
    'class_number_hint' => 'e.g. 1',
    'class_number_taken' => 'This class number is already used within this grade.',
    'needs_standardization' => ':count grade(s) need a standard number before promotion.',
    'standardize_now' => 'Standardize now',
    'standardization' => 'Standardize grades & classes',
    'standardization_intro' => 'Assign a standard number (1..12) to each grade and a number to each class. This one-time step makes the school promotion-ready.',
    'needs_grade_number' => 'Needs number',
    'standardize_grade_collision' => 'Grade number collision (:grades) — assign a distinct number to each grade.',
    'standardize_class_collision' => 'Duplicate class numbers within grade: :grade.',
    'stage_primary' => 'Primary',
    'stage_intermediate' => 'Intermediate',
    'stage_secondary' => 'Secondary',
    'gender_male' => 'Boys',
    'gender_female' => 'Girls',
    'view_classes' => 'Classes',
    'books' => 'Books',
    'edit_class' => 'Edit class',
    'class_details' => 'Class details',
    'capacity_below_students' => 'Capacity cannot be lower than the number of currently enrolled students (:count).',
    'no_grades_yet' => 'No grade levels yet',
    'classes' => 'Classes',
    'classes_in_grade' => 'Classes in grade',
    'add_class' => 'Add class',
    'class_name' => 'Class name',
    'capacity' => 'Capacity',
    'vacancies' => 'Vacancies',
    'students' => 'Students',
    'lead_teacher' => 'Lead teacher',
    'no_classes_yet' => 'No classes yet',
    'class_has_students' => 'Cannot delete a class with enrolled students',
    'students_in_class' => 'Class students',
    'add_student_to_class' => 'Add student to class',
    'choose_student' => 'Choose a student',
    'student_added' => 'Student added',
    'transfer_to' => 'Transfer to',
    'transfer_selected' => 'Transfer selected',
    'students_transferred' => 'Students transferred',
    'no_students_in_class' => 'No students in this class',

    // Permissions
    'copy_permissions' => 'Copy Permissions',
    'permission_roles' => 'Pick a role',
    'permission_main_functions' => 'Main functions',
    'permission_sub_functions' => 'Sub functions',
    'permission_groups' => [
        'academic-years' => 'Academic Years',
        'attendance' => 'Attendance',
        'classes' => 'Classes',
        'exams' => 'Exams',
        'grades' => 'Grades',
        'reports' => 'Reports',
        'roles' => 'Roles',
        'schedules' => 'Schedules',
        'schools' => 'Schools',
        'sections' => 'Grade Levels',
        'settings' => 'Settings',
        'subjects' => 'Subjects',
        'users' => 'Users',
        'weekly-plans' => 'Weekly Plans',
    ],
    'permissions_pick_role' => 'Pick a role and function to see permissions',
    'permissions_pick_function' => 'Pick a function to see permissions',
    'permissions_autosave_hint' => 'Changes are saved automatically as you toggle each permission',
    'copy_source_school' => 'Copy from school',
    'permissions_copied' => 'Permissions copied successfully',
    'confirm_copy_permissions' => 'Existing permissions will be replaced with the copied ones. Continue?',

    // Errors
    'cannot_delete_has_users' => 'Cannot delete a school with linked users',

    // Pass/fail results (Phase 3)
    'results_title' => 'Student results (pass/fail)',
    'results_intro' => 'Set each student\'s result for this class, or use "Mark all passed".',
    'results_saved' => ':count enrollment result(s) saved.',
    'results_mark_all' => 'Mark all passed',
    'results_mark_all_confirm' => 'This marks every student in this class as passed. Continue?',
    'results_no_students' => 'No students are enrolled in this class.',
    'result' => 'Result',
    'result_passed' => 'Passed',
    'result_failed' => 'Failed',
    'result_pending' => 'Pending',
    'student_name' => 'Student name',

    // Automatic year-end promotion
    'promotion_title' => 'Automatic student promotion',
    'promotion_open' => 'Open promotion screen',
    'promotion_hint' => 'Promotes passing students to the next grade (ordinal +1) with class-number matching, and graduates the top grade.',
    'promotion_preview' => 'Preview',
    'promotion_execute' => 'Run promotion',
    'promotion_execute_confirm' => 'Yes, run promotion',
    'promotion_history' => 'Promotion history',
    'promotion_plan' => 'Promotion plan',
    'promotion_from' => 'From',
    'promotion_to' => 'To',
    'promotion_action' => 'Action',
    'promotion_promoted' => 'Promoted',
    'promotion_graduated' => 'Graduated',
    'promotion_overflow' => 'Over capacity',
    'promotion_overflow_moved' => 'Moved to next class',
    'promotion_not_moved' => 'Not moved',
    'promotion_stays' => 'Stays put',
    'promotion_nothing' => 'No passing students to promote between these two years.',
    'promotion_fix_blockers' => 'Fix the errors before running.',
    'promotion_confirm_title' => 'Confirm promotion',
    'promotion_confirm_body' => 'This actually moves students between grades and classes. It can be rolled back from the history. Enter your password to confirm.',
    'promotion_password' => 'Password',
    'promotion_blocker_class_number' => 'Class “:class” in “:grade” has no class number — assign one before promoting.',
    'promotion_warn_no_class' => 'No class number :number in “:grade” to receive :count student(s) — they stay put and the school admin is notified.',
    'promotion_bad_password' => 'Incorrect password.',
    'promotion_same_year' => 'Source and destination years must differ.',
    'promotion_already_run' => 'A promotion between these two years already ran. Roll it back first to re-run.',
    'promotion_done' => 'Promotion done ✅ — promoted: :promoted, graduated: :graduated, overflow: :overflow_moved, not moved: :not_moved.',
    'promotion_rolled_back' => 'Promotion rolled back successfully.',
    'promotion_rollback' => 'Roll back',
    'promotion_rollback_body' => 'This restores every student to their pre-promotion grade and class. Enter your password to confirm.',
    'promotion_rollback_confirm' => 'Yes, roll back',
    'promotion_rollback_invalid' => 'This batch cannot be rolled back.',
    'promotion_rollback_not_latest' => 'Only the latest executed promotion can be rolled back.',
    'promotion_history_title' => 'Promotion history',
    'promotion_no_batches' => 'No promotions yet.',
    'promotion_summary' => 'Summary',
    'promotion_executed_by' => 'Executed by',
    'promotion_status' => 'Status',
    'promotion_status_executed' => 'Executed',
    'promotion_status_rolled_back' => 'Rolled back',
    'promotion_notify_title' => 'Class capacity exceeded during promotion',
    'promotion_notify_body' => 'Could not promote :count student(s) into class number :number in “:grade” — no room.',
];
